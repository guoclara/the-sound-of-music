...
def main():
    ...
    train_data = np.concatenate(
        (blues[:80], classical[:80], country[:80], disco[:80], hiphop[:80],
        jazz[:80], metal[:80], pop[:80], reggae[:80], rock[:80]), axis = 0
    )

    train_labels = np.array(
        blues_labels[:80] + classical_labels[:80] + country_labels[:80] + disco_labels[:80] + hiphop_labels[:80] + 
        jazz_labels[:80] + metal_labels[:80] + pop_labels[:80] + reggae_labels[:80] + rock_labels[:80]
    )

    train_data, train_labels = shuffle_data(train_data, train_labels)

    validate_data = np.concatenate(
        (blues[80:90], classical[80:90], country[80:90], disco[80:90], hiphop[80:90], 
        jazz[80:90], metal[80:90], pop[80:90], reggae[80:90], rock[80:90]), axis = 0
    )
    
    validate_labels = np.array(
        blues_labels[80:90] + classical_labels[80:90] + country_labels[80:90] + disco_labels[80:90] + hiphop_labels[80:90] + 
        jazz_labels[80:90] + metal_labels[80:90] + pop_labels[80:90] + reggae_labels[80:90] + rock_labels[80:90]
    )
    
    validate_data, validate_labels = shuffle_data(validate_data, validate_labels)

    test_data = np.concatenate(
        (blues[90:], classical[90:], country[90:], disco[90:], hiphop[90:], 
        jazz[90:], metal[90:], pop[90:], reggae[90:], rock[90:]), axis = 0
    )

    test_labels = np.array(
        blues_labels[90:] + classical_labels[90:] + country_labels[90:] + disco_labels[90:] + hiphop_labels[90:] + 
        jazz_labels[90:] + metal_labels[90:] + pop_labels[90:] + reggae_labels[90:] + rock_labels[90:]
    )
